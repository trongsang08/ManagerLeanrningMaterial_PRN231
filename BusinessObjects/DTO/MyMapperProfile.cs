...
namespace BusinessObjects.DTO
{
    public class MyMapperProfile : Profile
    {
        public MyMapperProfile() 
        {
            ...
            CreateMap<User, UserDto>().ForMember(dest => dest.RoleName, opt => opt.MapFrom(source => source.Role.RoleName));
            CreateMap<UserDto, User>();
            CreateMap<ViewModels.UploadAssignmentViewModel, AssignmentDto>();
            CreateMap<Assignment, AssignmentDto>().ForMember(dest => dest.TeacherName, opt => opt.MapFrom(source => source.Uploader.Fullname));
